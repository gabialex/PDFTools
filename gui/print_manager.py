...
class PrintManager:
    ...
    def get_available_printers():
        """Get list of available printers, prioritizing physical ones"""
        system = platform.system()
        printers = []
        virtual_keywords = [
            'xps', 'pdf', 'fax', 'virtual', 'onenote', 'oxps',
            'microsoft print to pdf', 'adobe pdf', 'document writer'
        ]

        try:
            if system == "Windows":
                # Using Windows API for better printer type detection
                printers = []
                for printer in win32print.EnumPrinters(win32print.PRINTER_ENUM_LOCAL | win32print.PRINTER_ENUM_CONNECTIONS):
                    printer_name = printer[2]
                    # Skip printers with virtual keywords
                    if not any(kw in printer_name.lower() for kw in virtual_keywords):
                        printers.append(printer_name)
                
                # If no physical printers found, fall back to all printers
                if not printers:
                ...
                if result.returncode == 0:
                    all_printers = [line.split()[0] for line in result.stdout.splitlines()]
                    printers = [p for p in all_printers if not any(
                        kw in p.lower() for kw in virtual_keywords + ['airprint', 'cups-pdf']
                    )]
                    if not printers:  # Fallback to all printers
                ...
                if result.returncode == 0:
                    all_printers = [line.split()[0] for line in result.stdout.splitlines()]
                    printers = [p for p in all_printers if not any(
                        kw in p.lower() for kw in virtual_keywords + ['cups-pdf', 'print-to-file']
                    )]
                    if not printers:  # Fallback to all printers
                        printers = all_printers
        ...
        # Sort printers with physical ones first
        return sorted(printers, key=lambda x: any(kw in x.lower() for kw in virtual_keywords))  
    ...
    def show_print_dialog(self):
        ...
        printers = PrintManager.get_available_printers()  # Static method call
        ...
        # Try to find the first physical printer
        default_printer_index = 0
        virtual_keywords = ['xps', 'pdf', 'fax', 'virtual']
        for idx, printer in enumerate(printers):
            if not any(kw in printer.lower() for kw in virtual_keywords):
                default_printer_index = idx
                break

        # Configure the combobox
        printer_combo['values'] = printers
        printer_combo.current(default_printer_index)
